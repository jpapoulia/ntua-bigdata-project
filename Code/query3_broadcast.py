...
spark = SparkSession.builder.appName("Query3_Broadcast_Final").getOrCreate()
...
crime_with_zip = crime_2015.alias("crime").join(
    zip_code_df.alias("zip").hint("broadcast"), 
...
all_data = crime_with_zip.join(
    income_data.alias("income").hint("broadcast"), 
...
crimes_with_income = all_data.join(
    income_data.alias("income2").hint("broadcast"), 
...
top_3_crimes = crimes_with_income.join(top_3_zipcodes.hint("broadcast"), crimes_with_income["ZIPcode"] == top_3_zipcodes["ZIPcode"], "inner")
bottom_3_crimes = crimes_with_income.join(bottom_3_zipcodes.hint("broadcast"), crimes_with_income["ZIPcode"] == bottom_3_zipcodes["ZIPcode"], "inner")
