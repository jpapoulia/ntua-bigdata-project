...
spark = SparkSession.builder.appName("Query3_Shuffle_Replicate_NL_Final").getOrCreate()
...
crime_with_zip = crime_2015.alias("crime").join(
    zip_code_df.alias("zip").hint("shuffle_replicate_nl"), 
...
all_data = crime_with_zip.join(
    income_data.alias("income").hint("shuffle_replicate_nl"), 
...
crimes_with_income = all_data.join(
    income_data.alias("income2").hint("shuffle_replicate_nl"), 
...
top_3_income_values = get_distinct_top_bottom(crimes_with_income, "Estimated Median Income", desc("Estimated Median Income"))
...
bottom_3_income_values = get_distinct_top_bottom(crimes_with_income, "Estimated Median Income", asc("Estimated Median Income"))
...
top_3_crimes = crimes_with_income.join(top_3_zipcodes.hint("shuffle_replicate_nl"), crimes_with_income["ZIPcode"] == top_3_zipcodes["ZIPcode"], "inner")
bottom_3_crimes = crimes_with_income.join(bottom_3_zipcodes.hint("shuffle_replicate_nl"), crimes_with_income["ZIPcode"] == bottom_3_zipcodes["ZIPcode"], "inner")
